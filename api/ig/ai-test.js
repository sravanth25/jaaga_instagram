...
export const DEFAULT_JAAGA_SYSTEM_PROMPT = `You are JaaGa's AI Assistant, a friendly and helpful chatbot for JaaGa. JaaGa specializes in Property Documents and Services across Telangana, India. You answer customer questions on WhatsApp, Instagram DMs, and the JaaGa website.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const activePrompt = (systemPrompt || '').trim() || DEFAULT_JAAGA_SYSTEM_PROMPT;
    ...
    if (!apiKey) {
      return res.status(200).json({
        ...
        raw: "Fallback: GEMINI_API_KEY environment variable not set on server."
      });
    }
    ...
    let cleanReply = rawText.replace(/\*/g, '').trim();

    if (!cleanReply) {
      cleanReply = "Thanks for messaging JaaGa! Our team will get back to you shortly. Visit https://www.jaaga.ai or call +91 88851 66880.";
    }
